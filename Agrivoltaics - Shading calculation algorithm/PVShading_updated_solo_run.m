function [surface_shade_in_CU_sum_final,surface_shade_in_CU_perc,...
    ...
    reply_array_end,sc_line_sh,PV_sh,CU_sh] = ...
    PVShading_updated_solo_run(yr,mth,d,hr,mnt,sc,timezone,...
	...
	elseif ~isnan(x_sun) && ~isnan(y_sun) && ~isnan(z_sun)
                                        ...
                                        for j=pos_PV{m,i}
                                            for t=1:num_sc_lines
                                                if ~isempty(Xi_shad_cm_in_CU{m,i}{j})
                                                    Xi_shad_cm_in_CU{m,i}{j}(Xi_shad_cm_in_CU{m,i}{j}(:) < min(Greenhouse_corners{1,2}(:,1))) =...
                                                        min(Greenhouse_corners{1,2}(:,1));
				                                    Yi_shad_cm_in_CU{m,i}{j}(Xi_shad_cm_in_CU{m,i}{j}(:) < min(Greenhouse_corners{1,2}(:,2))) =...
                                                        min(Greenhouse_corners{1,2}(:,2));
                                
				                                    Xi_shad_cm_in_CU{m,i}{j}(Xi_shad_cm_in_CU{m,i}{j}(:) > max(Greenhouse_corners{1,2}(:,1))) =...
                                                        max(Greenhouse_corners{1,2}(:,1));
				                                    Yi_shad_cm_in_CU{m,i}{j}(Yi_shad_cm_in_CU{m,i}{j}(:) > max(Greenhouse_corners{1,2}(:,2))) =...
                                                        max(Greenhouse_corners{1,2}(:,2));
                                    
				                                    Xi_shad_sc_cm_in_CU{m,i}{j}{:,t}(Xi_shad_sc_cm_in_CU{m,i}{j}{:,t}(:) < min(Greenhouse_corners{1,2}(:,1))) =...
                                                        min(Greenhouse_corners{1,2}(:,1));
				                                    Yi_shad_sc_cm_in_CU{m,i}{j}{:,t}(Yi_shad_sc_cm_in_CU{m,i}{j}{:,t}(:) < min(Greenhouse_corners{1,2}(:,2))) =...
                                                        min(Greenhouse_corners{1,2}(:,2));
                                
				                                    Xi_shad_sc_cm_in_CU{m,i}{j}{:,t}(Xi_shad_sc_cm_in_CU{m,i}{j}{:,t}(:) > max(Greenhouse_corners{1,2}(:,1))) =...
                                                        max(Greenhouse_corners{1,2}(:,1));
				                                    Yi_shad_sc_cm_in_CU{m,i}{j}{:,t}(Yi_shad_sc_cm_in_CU{m,i}{j}{:,t}(:) > max(Greenhouse_corners{1,2}(:,2))) =...
                                                        max(Greenhouse_corners{1,2}(:,2));
                                                else
								...
								for i=1:size(CU_tables,2)
									for m=1:size(CU_tables,1)
										for j=pos_PV{m,i}
											for t=1:num_sc_lines
												if ~isempty(Xi_shad_cm_in_CU{m,i}{j})
													Xi_shad_cm_in_CU{m,i}{j}(Xi_shad_cm_in_CU{m,i}{j}(:) < min(Greenhouse_corners{1,1}(:,1))) =...
														min(Greenhouse_corners{1,1}(:,1));
													Yi_shad_cm_in_CU{m,i}{j}(Xi_shad_cm_in_CU{m,i}{j}(:) < min(Greenhouse_corners{1,1}(:,2))) =...
														min(Greenhouse_corners{1,1}(:,2));
														
													Xi_shad_cm_in_CU{m,i}{j}(Xi_shad_cm_in_CU{m,i}{j}(:) > max(Greenhouse_corners{1,1}(:,1))) =...
														max(Greenhouse_corners{1,1}(:,1));
													Yi_shad_cm_in_CU{m,i}{j}(Yi_shad_cm_in_CU{m,i}{j}(:) > max(Greenhouse_corners{1,1}(:,2))) =...
														max(Greenhouse_corners{1,1}(:,2));
														
													Xi_shad_sc_cm_in_CU{m,i}{j}{:,t}(Xi_shad_sc_cm_in_CU{m,i}{j}{:,t}(:) < min(Greenhouse_corners{1,1}(:,1))) =...
														min(Greenhouse_corners{1,1}(:,1));
													Yi_shad_sc_cm_in_CU{m,i}{j}{:,t}(Yi_shad_sc_cm_in_CU{m,i}{j}{:,t}(:) < min(Greenhouse_corners{1,1}(:,2))) =...
														min(Greenhouse_corners{1,1}(:,2));
														
													Xi_shad_sc_cm_in_CU{m,i}{j}{:,t}(Xi_shad_sc_cm_in_CU{m,i}{j}{:,t}(:) > max(Greenhouse_corners{1,1}(:,1))) =...
														max(Greenhouse_corners{1,1}(:,1));
													Yi_shad_sc_cm_in_CU{m,i}{j}{:,t}(Yi_shad_sc_cm_in_CU{m,i}{j}{:,t}(:) > max(Greenhouse_corners{1,1}(:,2))) =...
														max(Greenhouse_corners{1,1}(:,2));
												else
													Xi_shad_cm_in_CU{m,i}{j} = Xi_shad_cm_in_CU{m,i}{j};
													Yi_shad_cm_in_CU{m,i}{j} = Yi_shad_cm_in_CU{m,i}{j};
		...
		for i=1:size(CU_tables,2)
            for m=1:size(CU_tables,1)
                trisurf(CU_bounds_3d,CU_arrays{m,i}(:,1),CU_arrays{m,i}(:,2),CU_arrays{m,i}(:,3),...
                    'Facecolor','red','FaceAlpha',0.1,'LineWidth',1)
                hold on
				for j=pos_PV{m,i}
                    surf(Xii{m,i}{j},Yii{m,i}{j},Zii{m,i}{j},'FaceColor','k','FaceAlpha',0.2);
                    hold on
                    surf(Xi_shad{m,i}{j},Yi_shad{m,i}{j},Zi_shad{m,i}{j},'FaceColor','k','FaceAlpha',0.1);
                    hold on
                    for t=1:num_sc_lines
                        surf(Xii_sc{m,i}{j}{t,1},Yii_sc{m,i}{j}{t,1},Zii_sc{m,i}{j}{t,1},'FaceColor','b','FaceAlpha',0.2);
                        hold on
                        surf(Xi_shad_sc{m,i}{j}{t},Yi_shad_sc{m,i}{j}{t},Zi_shad_sc{m,i}{j}{t},'FaceColor','b','FaceAlpha',1);
                    end
                end
			end
		end
        ...
        grid on
        box on
		% saveas(fig_3D, sprintf('C:\\Users\\thodo\\Desktop\\graphs_minutely\\jun_15\\pyr_2\\3D\\%d_%d_%d__%d_%d_%d.jpg', d, mth, yr, hr, mnt, sc), 'jpg')
        ...
        for i=1:size(CU_tables,2)
            for m=1:size(CU_tables,1)
                plot(Greenhouse_corners{m,i}(bounds_greenhouse,1),Greenhouse_corners{m,i}(bounds_greenhouse,2),...
                    'Color','b','LineWidth',2) 
                hold on
                for j=pos_PV{m,i}
                    plot(Xi_shad_cm{m,i}{j}(bounds_shade),Yi_shad_cm{m,i}{j}(bounds_shade),...
                        'Color','k','LineWidth',1.5)
                    hold on
                    for t=1:num_sc_lines
                        plot(Xi_shad_sc_cm{m,i}{j}{1,t}(bounds_shade),Yi_shad_sc_cm{m,i}{j}{1,t}(bounds_shade),...
		...
		% for the existance of the pyranometer
		if isempty(varargin)
            return;
		...
		elseif ~isempty(varargin) && mod(length(varargin),2) == 0
			for p = 1:2:numel(varargin)
				...
				for i = 1:size(CU_tables,2)
                    for m = 1:size(CU_tables,1)
                        for j = pos_PV{m,i}
							s_t_total{m,i}{j} = cell(1,num_sc_lines);
							
							for t = 1:num_sc_lines
								[~,s_ref{m,i}{j}{t}] = boundary(reshape(Xi_shad_sc_cm_in_all{m,i}{j}{t},[],1),...
                                    reshape(Yi_shad_sc_cm_in_all{m,i}{j}{t},[],1));
                                
                                [~,s_t_1{m,i}{j}{t}] = boundary(reshape(horzcat(Xi_shad_sc_cm_in_all{m,i}{j}{t}(:,1),x_st_1(:,1)),[],1),...
                                    reshape(horzcat(Yi_shad_sc_cm_in_all{m,i}{j}{t}(:,1),y_st_1(:,1)),[],1));

                                [~,s_t_2{m,i}{j}{t}] = boundary(reshape(horzcat(Xi_shad_sc_cm_in_all{m,i}{j}{t}(1,:),x_st_1(1,:)),[],1),...
                                    reshape(horzcat(Yi_shad_sc_cm_in_all{m,i}{j}{t}(1,:),y_st_1(1,:)),[],1));

                                [~,s_t_3{m,i}{j}{t}] = boundary(reshape(horzcat(Xi_shad_sc_cm_in_all{m,i}{j}{t}(:,2),x_st_1(:,2)),[],1),...
                                    reshape(horzcat(Yi_shad_sc_cm_in_all{m,i}{j}{t}(:,2),y_st_1(:,2)),[],1));

                                [~,s_t_4{m,i}{j}{t}] = boundary(reshape(horzcat(Xi_shad_sc_cm_in_all{m,i}{j}{t}(2,:),x_st_1(2,:)),[],1),...
                                    reshape(horzcat(Yi_shad_sc_cm_in_all{m,i}{j}{t}(2,:),y_st_1(2,:)),[],1));
									
								s_t_total{m,i}{j}{t} = s_t_1{m,i}{j}{t} + s_t_2{m,i}{j}{t} + s_t_3{m,i}{j}{t} + s_t_4{m,i}{j}{t};
								
								if s_ref{m,i}{j}{t} ~= 0 && s_ref{m,i}{j}{t} == s_t_total{m,i}{j}{t}
                                    reply{m,i}{j}{t} = 1;
                                elseif s_ref{m,i}{j}{t} == 0 || s_ref{m,i}{j}{t} ~= s_t_total{m,i}{j}{t}
                                    reply{m,i}{j}{t} = 0;
                                end
							end
						end
                ...
                title(string(datetime(yr,mth,d,hr,mnt,sc,'Format','dd-MMM-uuuu HH:mm:ss')),"FontWeight","bold","FontSize",15);
                box on
                % saveas(fig_2D_in_grh,sprintf('C:\\Users\\thodo\\Desktop\\graphs_minutely\\jun_15\\pyr_2\\2D\\%d_%d_%d__%d_%d_%d.jpg', d, mth, yr, hr, mnt, sc), 'jpg')
			end
			...
			for i = 1:size(CU_tables,2)
                for m = 1:size(CU_tables,1)
                    for j = pos_PV{m,i}
                        for t = 1:num_sc_lines
                            reply_array(t,j,m,i) = reply{m,i}{j}{t};                            
                        end
                    end
                end
            end
                        
            if any(reply_array(:) == 1)
                reply_array_end = 1;
                [sc_line_sh,PV_sh,CU_sh] = ind2sub(size(reply_array),find(reply_array(:) == 1));
            else
                reply_array_end = 0;
                sc_line_sh = NaN;
                PV_sh = NaN;
			...
			if ~isempty(find(reply_array(:) == 1, 1))
                report = sprintf(['The shaded area inside greenhouse unit %d at a height of %0.2f m by the solar cells is equal to %.2f m2, while it covers %.2f%% of it. ' ...
                    'The point corresponding to the coordinates (%0.2f,%0.2f)m is inside the shaded area and is shaded by' ...
                    ' the line of solar cells %d, the photovoltaic %d on the roof of greenhouse unit %d.'],...
                    size(CU_tables,2),z_shad,round(surface_shade_sc_in_CU_sum_final,2,'decimals'),round(surface_shade_sc_in_CU_perc,2,'decimals'),...
                    x_st/100,y_st/100,sc_line_sh,PV_sh,CU_sh);
                disp(report)
            else
                report = sprintf(['The shaded area inside greenhouse unit %d at a height of %0.2f m by the solar cells is equal to %.2f m2, while it covers %.2f%% of it. ' ...
                    'The point corresponding to the coordinates (%0.2f,%0.2f)m is outside the shaded area.'],...
                    size(CU_tables,2),z_shad,round(surface_shade_sc_in_CU_sum_final,2,'decimals'),round(surface_shade_sc_in_CU_perc,2,'decimals'),...
                    x_st/100,y_st/100);
                disp(report)
            end
